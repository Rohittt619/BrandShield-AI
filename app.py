...
# Add project root, src and database directories to sys.path
...
# ─────────────────────────────────────────────
# Helper: render analysis results from session state
# ─────────────────────────────────────────────
def render_results():
    ...
    if results["is_authentic"]:
        st.success(f"✅  AUTHENTIC — Logo matches **{target_brand}** specifications  |  Score: **{score}%**  |  Threat: **{threat}**")
    else:
        st.error(f"❌  COUNTERFEIT / ALTERED — High-risk logo detected for **{target_brand}**  |  Score: **{score}%**  |  Threat: **{threat}**")
    ...
    st.download_button(
        label="📥 Download Executive Forensic Verification Certificate (PDF)",
        data=pdf_bytes,
...
def main():
    ...
    for key in ("results", "original_image"):
    ...
    if nav == "🏗️ System Architecture":
        ...
        3. **Google Gemini 1.5 Vision** — Brand Mismatch detection, Typography & Symmetry audit
    ...
    if nav == "📷 Live Webcam Scanner":
        ...
        if camera_photo:
            ...
            with st.spinner(f"Running forensic inspection for {target_brand}…"):
                ...
                db.save_inspection(
                    brand=target_brand, verdict=results["verdict_label"],
    ...
    # ═══════════════════════════════════════════
    # TAB: File Upload Inspection (DEFAULT)
    # ═══════════════════════════════════════════
    ...
    # Always render results from session_state OUTSIDE the button block
    render_results()
