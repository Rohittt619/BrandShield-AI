...
class BrandShieldDetector:
    ...
    def analyze_logo(self, image_pil: Image.Image, target_brand: str = "Nike") -> dict:
        ...
        if ai_verdict and isinstance(ai_verdict, dict):
            ...
            forensic_summary = [
                ...
                "ℹ️ Add GEMINI_API_KEY in Streamlit Secrets for full brand-mismatch AI detection.",
            ]
    ...
    def _run_gemini_vision_audit(self, image_pil: Image.Image, target_brand: str) -> dict | None:
        try:
            from google.genai import types

            # Convert PIL image → bytes for the new SDK
            buf = io.BytesIO()
...
1. BRAND MISMATCH CHECK: If the logo belongs to a DIFFERENT brand than "{target_brand}", set verdict="COUNTERFEIT", authenticity_score=5.0, threat_level="HIGH", and note the mismatch.
